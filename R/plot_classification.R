...
plot.binary_clf <- function(train_output,
                            models = NULL,
                            type   = 'comparison',
                            metric = 'accuracy'){
  ...
  if (type == 'comparison') {
    ...
    test_data      <- train_output$score_test[1:10, ]
    ...
    valid_scores    <- data.frame(t(train_output$score_valid[, (NCOL(train_output$score_valid) - 2):NCOL(train_output$score_valid)]))
    valid_scores    <- valid_scores[, 1:10]
    valid_y         <- data.frame(metric = rownames(valid_scores), value = unlist(valid_scores))
    valid_data      <- train_output$score_valid[1:10, ]
  ...
  if (type == 'roc') {
    suppressMessages(
      test_rocobj  <- pROC::roc(train_output$test_observed,
                                train_output$predictions_test[[train_output$score_test$name[[1]]]]))
    suppressMessages(
      test_auc     <- round(pROC::auc(train_output$test_observed,
                                      train_output$predictions_test[[train_output$score_test$name[[1]]]]), 4))
    suppressMessages(
      valid_rocobj <- pROC::roc(train_output$valid_observed,
                                train_output$predictions_valid[[train_output$score_valid$name[[1]]]]))
    suppressMessages(
      valid_auc    <- round(pROC::auc(train_output$valid_observed,
                                      train_output$predictions_valid[[train_output$score_valid$name[[1]]]]), 4))
    ...
    roc_plot <- function(test) {
      if (test) {
        ...
        auc     <- test_auc
        text    <- 'testing'
        name    <- train_output$score_test$name[[1]]
      } else {
        rocobj  <- valid_rocobj
        auc     <- valid_auc
        text    <- 'validation'
        name    <- train_output$score_valid$name[[1]]
      }

      p <- pROC::ggroc(rocobj, colour = colors_discrete_forester(1), size = 2) +
        theme_forester() +
        labs(
  ...
  if(type == 'train-test') {
    ...
    test_score <- train_output$score_test[train_output$score_test$name %in% models_names, c('accuracy', 'auc', 'f1')]
