...
check_outliers <- function(df, verbose = TRUE) {
  ...
  if (length(outliers) == 0) {
    ...
    verbose_cat(crayon::red('\u2716'), 'These observations might be outliers due to their numerical column values: \n', outliers, ';\n', verbose = verbose)
    str <- capture.output(
      cat('**These observations might be outliers due to their numerical column values: **\n\n', outliers, ';\n'))
  } else {
