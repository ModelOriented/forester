test_that('test-preprocessing', {
  ...
  expect_true(length(bin_compas$Two_yr_Recidivism) == sum(as.integer(bin_compas$Two_yr_Recidivism %in% c(0, 1))))
  expect_true(is.null(levels(bin_lisbon$Price)))
  expect_true(length(levels(bin_iris$Species)) == 3)
  expect_true(0 == sum(as.integer(bin_adult$salary %in% c(0, 1))))
  expect_true(length(levels(bin_lymph$class)) == 4)
  expect_true(is.null(levels(bin_test$y)))

  labels_compas <- binarize_target(rm_stat_compas, type = 'binary_clf', 'Two_yr_Recidivism')$labels
  labels_lisbon <- binarize_target(rm_stat_lisbon, 'Price')$labels
  labels_iris   <- binarize_target(rm_stat_iris, 'Species')$labels
  labels_adult  <- binarize_target(rm_stat_adult, type = 'binary_clf', 'salary')$labels
